...
nnls(A,b) = nonneg_lsq(A,b;alg=:nnls,Gram=false)
nnls_gram(A,b) = nonneg_lsq(A,b;alg=:nnls,Gram=true)
...
# Test that nnls_gram produces the same solution as nnls and scipy
x = [0.15512102, 0.69328985] # approx solution from scipy
@test norm(nnls_gram(AA,Ab)-x) < 1e-5
@test norm(nnls_gram(AA,Ab)-nnls(A,b)) < 1e-5
...
@test norm(nnls_gram(AA2,Ab2)-x2) < 1e-5
@test norm(nnls_gram(AA2,Ab2)-nnls(A2,b2)) < 1e-5
...
for i = 1:10
	...
	@test norm(nnls_gram(AA3,Ab3)-x3) < 1e-5
	@test norm(nnls_gram(AA3,Ab3)-nnls(A3,b3)) < 1e-5
end
